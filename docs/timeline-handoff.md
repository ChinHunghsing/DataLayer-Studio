...
- 当前提交：轨道头支持独立选择多个切割目标并再次点击取消；⌘B 有选中轨道时只分割这些轨道，未选择轨道时分割播放头下全部未锁定轨道。
...
- 点击轨道头可独立选择或取消多个切割目标并显示高亮；删除选中的空轨会清理对应选择，Undo 恢复轨道时也恢复选择集合。
- ⌘B 在播放头处分割：有选中轨道时只分割这些轨道，未选择轨道时分割播放头下全部未锁定轨道（两侧源内容连续，左片保留原 ID 和选中态）；⌫ 删除选中片段并保留间隙；⇧⌫ 波纹删除，在所有未锁定轨道上闭合被删时间段，跨段片段自动裁剪或拆分（锁定轨不动）。入口有「时间线」菜单和片段右键菜单，实现在 `Sources/OverlayCore/Timeline/TimelineEditing.swift`。
...
- 无轨道选择时全轨分割、选中轨道时定向分割、轨道选择切换，以及删轨/Undo 时选择状态修复。
...
- `swift test`：355 个测试全部通过，0 failure。
- `swift test --filter MediaPoolTests`：46 个测试全部通过。
- `swift test --filter AudioWaveformLoaderTests`：2 个测试全部通过；另用本地真实 AAC 视频验证了流式峰值抽取。
- `swift test --filter TimelineModelTests`：21 个测试全部通过。
- `swift test --filter TimelineVideoWriterTests`：11 个测试全部通过，包含稀疏视频、黑帧、音频静音区间和透明空白帧的真实写出检查。
...
7. 不选择轨道时在多条重叠轨道中按 ⌘B，确认播放头下所有未锁定轨都被分割；点击轨道头选中一轨后再按 ⌘B，确认只分割该轨；再选一轨确认两条都会分割；逐个取消选择后恢复全轨分割。
8. 把 `single.*` 视频和运动片段都拖到 0 秒之后，确认开头是空白黑画布，且“同步”标签中的源匹配时间不被拖动修改。
9. 修改同步标签的“视频时间/运动时间”，确认两素材对应源时刻重新落在同一个时间线位置，同时保留完整素材开头。
