...
final class StudioModelTests: XCTestCase {
    ...
    func testPendingAlignmentExportConfirmationRespectsModeRangeAndRenderScope() {
        let activity = MediaAsset(
            id: "activity",
            kind: .activity,
            url: URL(fileURLWithPath: "/tmp/activity.fit"),
        ...
        let project = TimelineProject(
            ...
            tracks: [
                ...
                TimelineTrack(id: "v1", kind: .video, name: "V1", clips: [
                    TimelineClip(id: "video-clip", assetID: video.id, timelineStart: 30, duration: 10)
                ])
            ]
        )

        XCTAssertTrue(StudioModel.requiresPendingAlignmentExportConfirmation(
            project: project,
            mode: .overlay,
        ...
        XCTAssertFalse(StudioModel.requiresPendingAlignmentExportConfirmation(
            project: project,
            mode: .overlay,
            renderScope: .singleClip,
            timelineStart: 20,
    ...
    func testExportRequestsConfirmationBeforeChoosingOutputForPendingAlignment() {
        let model = StudioModel()
        let base = Date(timeIntervalSince1970: 1_750_000_000)
        let referenceURL = URL(fileURLWithPath: "/tmp/reference-alignment.fit")
        let referenceSeries = TelemetrySeries(samples: [
            TelemetrySample(elapsed: 0, date: base),
            TelemetrySample(elapsed: 20, date: base.addingTimeInterval(20))
        ])
        model.upsertActivityAsset(url: referenceURL, series: referenceSeries)
        model.addActivityAssetToTimeline(id: referenceURL.path)

        let activityURL = URL(fileURLWithPath: "/tmp/pending-alignment.fit")
        let series = TelemetrySeries(samples: [
            TelemetrySample(elapsed: 0),
            TelemetrySample(elapsed: 20)
        ])
        model.upsertActivityAsset(url: activityURL, series: series)
        model.addActivityAssetToTimeline(id: activityURL.path)
        for clipID in model.timeline.tracks.flatMap(\.clips).map(\.id) {
            model.setTimelineClipLayout(id: clipID, .default)
        }
        XCTAssertEqual(model.timeline.tracks.flatMap(\.clips).filter(\.isAlignmentPending).count, 1)
        XCTAssertNil(model.exportReadinessMessage)

        model.export()

        XCTAssertTrue(
            model.isPendingAlignmentExportConfirmationPresented,
            "status=\(model.status), weather=\(model.isWeatherExportConfirmationPresented)"
        )
        XCTAssertFalse(model.isExporting)
        XCTAssertNil(model.outputURL)
        ...
        XCTAssertFalse(model.isPendingAlignmentExportConfirmationPresented)
    }
    ...
    func testMissingWallClockMarksSingleSourcePairPendingUntilManualSync() {
        let base = Date(timeIntervalSince1970: 1_750_000_000)
        let model = loadedSingleSourceModel(
        ...
        XCTAssertTrue(model.timeline.tracks.flatMap(\.clips).allSatisfy(\.isAlignmentPending))

        model.syncVideoSeconds = 1

        XCTAssertTrue(model.timeline.tracks.flatMap(\.clips).allSatisfy { !$0.isAlignmentPending })
    }
