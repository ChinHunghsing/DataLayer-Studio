...
- `swift test`：380 个测试全部通过，0 failure。
- `swift test --filter MediaPoolTests`：58 个测试全部通过。
...
- `swift test --filter StudioModelTests`：69 个 macOS 状态模型测试全部通过。
...
1. 增加可见吸附线、吸附来源提示和多选反馈。
2. 处理混合横竖屏视频的逐片段 transform。
