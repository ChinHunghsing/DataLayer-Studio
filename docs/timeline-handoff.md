...
- 选中任意未锁定片段后，右侧检查器可编辑时间线开始、素材入点、时长；运动片段还可设置单片段距离单位和布局。
...
- `swift test`：318 个测试全部通过，0 failure。
- `swift test --filter MediaPoolTests`：29 个测试全部通过。
- `swift test --filter TimelineModelTests`：13 个测试全部通过。
...
- 删除选中片段（⌫ 留隙 / ⇧⌫ 波纹删除）已实现，但没有确认与 Undo；误删只能重新从媒体池加入。
...
4. 分别移动和裁剪视频、运动数据片段，检查吸附、检查器数值和预览源时间。
5. 把 `single.*` 视频和运动片段都拖到 0 秒之后，确认开头是空白黑画布，且“同步”标签中的源匹配时间不被拖动修改。
6. 修改同步标签的“视频时间/运动时间”，确认两素材对应源时刻重新落在同一个时间线位置，同时保留完整素材开头。
7. 调整导出 in/out，确认播放头和预览范围 clamp 正确。
8. 分别导出透明浮层和合成视频。
