...
- 当前提交：时间线工具栏可显式新增 V2、V3 等视频轨；视频可从媒体池拖到任意视频轨，空视频轨可删除，新增/删除均支持 Undo/Redo。
...
- 多条视频轨目前用于组织顺序片段；所有已启用视频轨之间仍不允许时间重叠，片段也不能超出源视频可用时长。画中画或“上层视频覆盖下层视频”的合成语义尚未引入。
...
- 任意已启用视频轨之间的 overlap 仍是错误；多视频轨已用于组织顺序片段，但没有引入画中画、上层覆盖、转场等视频合成语义。
...
4. 连续新增 V2、V3，把视频从媒体池分别拖入目标轨，确认轨道位于 overlay 下方且名称递增；删除片段使轨道变空后删除该轨，并检查 Undo/Redo。再打开一个包含足够多 overlay 轨的工程，确认轨道区可纵向滚动。
