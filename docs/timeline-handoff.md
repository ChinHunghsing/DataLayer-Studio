...
1. 为工程 JSON 增加 schema/version 和向后兼容迁移入口，并确定导出 in/out、activity trim、渲染范围等工程级设置的持久化边界。
2. 增加工程目录相对路径 fallback；bookmark、原绝对路径、相对路径都失败后才进入当前离线/relink 流程。
3. 完善轨道重命名、启用、锁定和 overlay 空轨新增/删除交互，再处理片段跨同类轨移动。
4. 增加可见吸附线、吸附来源提示和多选反馈。
5. 最后处理混合横竖屏视频的逐片段 transform；继续用 `assets/resourses/` 真实素材做回归，该目录和产物不得提交。
