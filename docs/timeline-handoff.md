...
- `swift test`：377 个测试全部通过，0 failure。
- `swift test --filter MediaPoolTests`：56 个测试全部通过。
...
1. 支持片段跨同类轨移动，并明确目标轨冲突时的落点规则。
2. 增加可见吸附线、吸附来源提示和多选反馈。
3. 处理混合横竖屏视频的逐片段 transform。
4. 用 `assets/resourses/` 真实素材复测工程保存/重开、相对路径、relink、两种导出和长时内存；该目录和产物不得提交。
...
7. 分别移动和裁剪视频、运动数据片段，检查吸附、检查器只读时间码和预览源时间。
