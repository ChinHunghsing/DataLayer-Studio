...
enum AppLocalizer {
    ...
    private static let english: [String: String] = [
        ...
        "workspace.timeline.hint": "Drag clips to move them independently; Command-click selects multiple clips. Use Sync to align source times, clip handles to trim, and the in/out band to set the export range. ⌘B splits selected clips, or all tracks when none are selected; ⌫ deletes selected clips and keeps gaps, ⇧⌫ ripple deletes.",
    ...
    private static let simplifiedChinese: [String: String] = [
        ...
        "workspace.timeline.hint": "拖动片段可独立调整位置，按住 Command 点击可多选片段；在“同步”中对齐源时间，用片段手柄裁剪，用 in/out 范围带设定导出范围。⌘B 分割播放头下的选中片段，未选片段时分割全部轨道；⌫ 删除选中片段并保留间隙，⇧⌫ 波纹删除。",
    ...
    private static let traditionalChinese: [String: String] = [
        ...
        "workspace.timeline.hint": "拖曳片段可獨立調整位置，按住 Command 點擊可多選片段；在「同步」中對齊來源時間，用片段手柄裁剪，用 in/out 範圍帶設定匯出範圍。⌘B 分割播放頭下的選取片段，未選片段時分割全部軌道；⌫ 刪除選取片段並保留間隙，⇧⌫ 波紋刪除。",
    ...
    private static let japanese: [String: String] = [
        ...
        "workspace.timeline.hint": "クリップは個別にドラッグ移動でき、Command キーを押しながらクリックすると複数選択できます。「同期」でソース時刻を合わせ、ハンドルでトリムし、イン／アウト範囲で書き出し範囲を設定します。⌘B は再生ヘッド上の選択クリップを分割し、未選択なら全トラックを分割します。⌫ で選択クリップを削除（ギャップは残る）、⇧⌫ でリップル削除。",
