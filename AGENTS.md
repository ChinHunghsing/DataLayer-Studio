...
- 发布时如果本地有用户未提交且与发布无关的改动，使用 `/private/tmp` 下从 `origin/main` 创建的 clean worktree 打 tag、核对 release 和做临时文件，避免误提交或污染用户工作区。
