...
- 打开工程时会按稳定的 `MediaAsset.id` 加载素材；文件不存在、不可读或解析失败都会进入统一的离线状态，不再因为 bookmark 解析到新路径而生成重复素材 ID。
- 单个素材离线时保留媒体池条目、轨道、片段 ID、时间线位置、裁切和浮层布局，工程其余可用素材继续加载。
- 媒体池显示“离线”状态和“重新链接”入口；时间线中的离线片段显示红色边框与警告图标。
...
- 四语言文案和 `MediaPoolTests` 已覆盖“离线工程仍保留片段”和“重新链接后稳定 ID/片段几何不变”。

当前状态是每个素材一个明确的在线/离线结果；尚未区分展示 bookmark stale、文件不存在、权限不足、格式损坏等更细原因，错误详情只在重新链接失败时显示。
...
- 继续扩大 Undo/Redo 覆盖面；当前片段移动、裁剪、分割、删除、加入、两类轨道新增、轨道状态修改和空轨删除已可撤销，媒体源替换/移除仍是确认后清空撤销栈。
