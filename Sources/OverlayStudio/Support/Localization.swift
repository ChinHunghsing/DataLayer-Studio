...
enum AppLocalizer {
    ...
    private static let english: [String: String] = [
        ...
        "workspace.timeline.hint": "Drag clips to move them independently; use Sync to align source times, clip handles to trim, and the in/out band to set the export range. ⌘B splits the selected clip, or all tracks when no clip is selected; ⌫ deletes the selected clip and keeps the gap, ⇧⌫ ripple deletes.",
    ...
    private static let simplifiedChinese: [String: String] = [
        ...
        "workspace.timeline.hint": "拖动片段可独立调整位置；在“同步”中对齐源时间，用片段手柄裁剪，用 in/out 范围带设定导出范围。⌘B 分割播放头下的选中片段，未选片段时分割全部轨道；⌫ 删除选中片段并保留间隙，⇧⌫ 波纹删除。",
    ...
    private static let traditionalChinese: [String: String] = [
        ...
        "workspace.timeline.hint": "拖曳片段可獨立調整位置；在「同步」中對齊來源時間，用片段手柄裁剪，用 in/out 範圍帶設定匯出範圍。⌘B 分割播放頭下的選取片段，未選片段時分割全部軌道；⌫ 刪除選取片段並保留間隙，⇧⌫ 波紋刪除。",
    ...
    private static let japanese: [String: String] = [
        ...
        "workspace.timeline.hint": "クリップは個別にドラッグ移動できます。「同期」でソース時刻を合わせ、ハンドルでトリムし、イン／アウト範囲で書き出し範囲を設定します。⌘B は再生ヘッド上の選択クリップを分割し、クリップ未選択なら全トラックを分割します。⌫ で選択クリップを削除（ギャップは残る）、⇧⌫ でリップル削除。",
