...
结论：阶段 4 至 9 的规划已全部完成。“标准剪辑逻辑、轻量交互”的时间线里程碑已经收口：支持稀疏多轨、上层覆盖、独立同步、多选/吸附/Undo、工程保存恢复、素材 relink 和透明/合成导出。后续应转入新里程碑，不再继续扩张基础时间线范围。
...
- `swift test`：385 个测试全部通过，0 failure。
- `swift test --filter MediaPoolTests`：61 个测试全部通过。
...
- 最终解码复核：暂停场景前 2 秒黑底/全透明，数据出现后亮度/Alpha 非零，暂停内 5 秒和 10 秒的 BGRA 像素哈希一致；完整输出的时长、画面抽样、Alpha、4K 尺寸和音轨落点均由 AVFoundation 实际解码断言通过。随后重新执行 `swift test`，385 项测试 0 failure，并重新通过 App bundle 构建和校验。
...
7. 分别把视频、运动数据片段上下拖到同类轨道，确认目标轨高亮、片段属性和选中态不变；尝试拖到锁定轨、不同类型轨和已有片段的位置，确认拒绝或落在最近空隙。再检查 Undo/Redo、吸附、检查器只读时间码和预览源时间。
8. 不选择片段时在多条重叠轨道中按 ⌘B，确认播放头下所有未锁定轨都被分割；选中播放头下某个片段后确认只分割该片段；选中远离播放头的片段时确认不会误切其他轨道。
9. 把 `single.*` 视频和运动片段都拖到 0 秒之后，确认开头是空白黑画布，且“同步”标签中的源匹配时间不被拖动修改。
10. 在唯一完整视频/运动片段组合中修改同步时间，确认两素材对应源时刻重新对齐；再分割或裁切任一片段，确认同步页变为不可用；恢复为各一个完整片段后重新可用。
