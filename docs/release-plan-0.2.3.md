...
- 行为约定：
  - 视频缺 creation time 元数据、或推算出的相对位置明显不合理（如相隔数天）时，回退到现有的「末尾顺延」行为并给出状态提示，不静默摆错。
  - 自动摆放结果必须可撤销（纳入现有时间线 Undo/Redo）。
  - 手动移动过的片段不被后续导入的自动对表重排。
...
| C2 | 多浮层透明边缘 QA | HEVC Alpha 与 ProRes 4444 多浮层输出放到深色/浅色/彩色背景做逐像素边缘检查；透明发灰是项目红线，此项从未验证 |
| C3 | 4K 长片压力回归 | 完整 4K/29.97fps 长时成片（≥110 分钟时间线）导出一次，确认内存不随时长增长；目前长时回归只做过 640×360@1fps |
