...
- 最终代码复测：使用当前 `StudioModel` 加载真实 `1.mp4` 和 FIT，保存 schema 2 JSON、创建新模型重开，再把两类素材 URL 改为缺失路径并 relink 回原文件；稳定素材 ID、两条轨道、片段几何和解析后的运动序列全部保持，测试通过。
- 最终导出复测：当前 release CLI 用三段真实视频再次输出 640×360、5 fps、10 秒 HEVC Alpha 浮层和 H.264 合成视频；两者时长均精确为 10 秒，合成结果保留 3 条 AAC 音轨。
- 最终长时复测：当前 release CLI 再次输出 640×360、1 fps、6639 秒 H.264 完整时间线，时长精确为 6639 秒且保留 3 条 AAC 音轨；500 秒抽样为纯黑，623、4000、6638 秒均检测到数据层。耗时 98.68 秒，最大常驻内存约 205 MiB、peak footprint 约 334 MiB，没有随 110 分钟时间线长度产生整段帧缓存。
