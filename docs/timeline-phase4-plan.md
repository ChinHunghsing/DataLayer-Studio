...
> 状态：**阶段 4.1-4.3 已完成（单源导出入口改由时间线驱动）**；**4.4 已完成当前会话内的 timeline 存储状态，自定义片段拖拽与左右裁剪会写回片段几何，尚未做工程持久化**；**4.5 已支持在 UI/模型中把素材池视频追加成顺序视频片段，但导出器尚未跨多个视频源切换**；**4.6 的透明多浮层与合成视频多浮层写出已完成**。当前实现仍保守复用既有单源写出器；多视频实际导出、Inspector 编辑多片段、吸附和 CLI 时间线导出仍未完成。
...
### 步骤 4.4　时间线变权威存储模型
**状态：部分完成。** `StudioModel` 已有 `@Published private(set) var timeline`，`currentTimelineProject` 读该存储状态；单源换源/同步/布局/输出设置会重建它。素材池中的运动文件和视频都可加入当前 timeline，进入自定义时间线后不会被输出设置变化覆盖。自定义片段拖拽会写回该 clip 的 `timelineStart`，非迁移片段左右裁剪会写回 `timelineStart/sourceIn/duration`。尚未完成：工程存档 Codable 兼容。
...
### 步骤 4.5　多视频顺序拼接
**状态：部分完成。** 素材池视频可追加到基础视频轨尾部，形成多个视频 clip；模型和 UI 已能表达顺序拼接。尚未完成：合成视频导出仍要求单个视频 clip，`TimelineVideoWriter` 还没有跨多个视频源逐帧切换。
...
**状态：部分完成。** 非迁移时间线片段已经支持左右裁剪，写回 clip 的几何字段；拖拽位置已有模型和 UI 支持。尚未完成：吸附、片段检查器、单段布局/距离单位编辑、CLI 暴露时间线导出。
