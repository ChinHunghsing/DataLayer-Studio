...
| # | 项目 | 说明 |
|---|------|------|
| C1 | 「完成时间线」错字 | ✅ 已修正。`renderScope.singleClip` 与 `help.renderScope` 的 zh-Hans/zh-Hant 已改为「完整时间线／完整時間線」 |
...
| C7 | 逆序自动对表撤销完整性 | ✅ 自动对表整体右移所影响的时间线、手动导出范围与播放头纳入同一撤销步骤，补回归测试 |
| C8 | 运动类型生命周期 | ✅ 工程恢复/素材重连保留 FIT/GPX sport；媒体池撤销/重做同步恢复 sport，补回归测试 |
| C9 | 时间线头部与轨道控制布局 | ✅ 改为专业紧凑的单行工具栏；长操作提示收进帮助弹层，轨道类型/名称去重，显隐与锁定控制对齐 |
| C10 | 时间线片段启用/禁用 | ✅ 单个片段可通过右键菜单或达芬奇同款 `D` 快捷键切换；预览与导出统一跳过被禁用的轨道和片段，旧工程片段默认启用 |
