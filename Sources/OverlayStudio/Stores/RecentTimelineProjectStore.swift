...
final class RecentTimelineProjectStore {
    ...
    func load() -> [RecentTimelineProject] {
        ...
        for item in stored {
            ...
            projects.append(RecentTimelineProject(url: url))
            refreshed.append(StoredProject(path: path, bookmarkData: resolved.bookmarkData))
        }
    ...
    func record(_ url: URL) -> [RecentTimelineProject] {
        ...
        stored.insert(StoredProject(path: path, bookmarkData: bookmarkData), at: 0)
